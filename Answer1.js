function findCaliforniaCafes(parameter) {
    ...
    const places = [{
        ...
        // mapping through cafes to find the cafe name
        cafes.map((elem)=>{
            let cond = elem.name.includes(parameter);
            if(cond ==true){
                // Mapping Through places to get details
                places.map((el)=>{
                        if (elem.place_id==el.id) {  
...
console.log(findCaliforniaCafes("Ave"));